...
namespace pro_function
{
	...
	void draw(rst::rasterizer *r, std::vector<std::shared_ptr<Triangle>> &divide)
	{
		r->draw(divide);
	}

	void allocate_ThreadPool(int Pool_size, std::vector<std::vector<std::shared_ptr<Triangle>>> divide_lists, Thread::ThreadPool *pools, rst::rasterizer *r)
	{
		//r.draw(TriangleList);
        std::vector<std::shared_ptr<Triangle>> divide;
		for (int i = 0; i < Pool_size; i++)
		{
			divide = divide_lists[i];
			pools->enqueue(pro_function::draw,r,divide);
		}
		pools->wait_for_tasks();    //�ȴ������߳����
	}
}
